	...
	<div class="relative">
	  ...
	  <div class="relative group">
		...
		<a href={`/product/${product.id}`} class="relative inline-block">
			<img class="w-96 h-96" alt={product.title} src={product.images[0]} />
			{#if product.images[1]}
			  <img class="w-96 h-96 absolute top-0 left-0 opacity-0 hover:opacity-100 transition-opacity duration-300" alt={product.title} src={product.images[1]} />
			{/if}
		  </a>		  
		<div class="absolute bottom-0 p-8 w-full opacity-0 group-hover:opacity-100">
		  <button class="dark:bg-gray-800 dark:text-gray-300 font-medium text-base leading-4 text-gray-800 bg-white py-3 w-full" on:click={() => addToCart(product.id, totalPrice, price)}>Add to bag</button>
		  <a href={`/product/${product.id}`}>
			<button class="bg-transparent font-medium text-base leading-4 border-2 border-white py-3 w-full mt-2 text-black">Quick View</button>
		  </a>
		</div>
	  </div>
	  <a href={`/product/${product.id}`}>
